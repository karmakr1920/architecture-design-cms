	...
	function twenty_twenty_one_setup() {
		...
		register_nav_menus(
			array(
				...
				'policy' => esc_html__( 'Policy menu', 'twentytwentyone' ),
...
function add_menu_icon($items, $args) {
    if ($args->theme_location == 'footer') {
		$items = preg_replace('/<a (.*?)>(.*?)<\/a>/', '<a $1><i class="fas fa-chevron-right"></i>$2</a>', $items);
    }
    return $items;
}
...
function modify_news_query( $query ) {
    if ( is_post_type_archive( 'news' ) && !is_admin() && $query->is_main_query() ) {
        $query->set( 'posts_per_page', 10 ); // Set the number of posts per page
    }
}
add_action( 'pre_get_posts', 'modify_news_query' );
